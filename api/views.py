...
def dashboard(request):
    ...
    return render(request, "api/index.html", context=add_sidebar_context(context))
...
def budget(request, budget_name):
    ...
    return render(request, "api/budget.html", context=add_sidebar_context(context))
...
def add_money(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(request, f'Added {form.cleaned_data.get("amount")} to budgets!')
            # if ANYTHING went wrong here, let them know
            #except:
            #    messages.warning(request, f'Failed to add {form.cleaned_data.get("amount")}')
        else:
            messages.warning(request, f'Invalid input')


        return render(request, "api/add_money.html", context=add_sidebar_context(context))
    else:
