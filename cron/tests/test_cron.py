...
from cron.management.commands.run_cron_daily_jobs import Command as RunCronJobCommand


class Test(BudgetTestCase):
    def test_daily_command(self):
        with patch(
            "cron.management.commands.run_cron_daily_jobs.CronJobRunner"
        ) as mock_runner:
            RunCronJobCommand().handle()
        mock_runner.execute_cron_daily_jobs.assert_called_once()

    def test_monthly_command(self):
        with patch(
                "cron.management.commands.run_cron_monthly_jobs.CronJobRunner"
        ) as mock_runner:
            RunCronJobCommand().handle()
        mock_runner.execute_cron_monthly_jobs.assert_called_once()
